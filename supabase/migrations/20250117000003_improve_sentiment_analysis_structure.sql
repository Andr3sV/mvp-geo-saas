-- =============================================
-- MIGRATION: Improve Sentiment Analysis Structure
-- Description: Better normalization with proper references
-- Date: 2025-01-17
-- =============================================

-- Drop and recreate sentiment_analysis table with better structure
...
CREATE TABLE sentiment_analysis (
  ...
  -- Entity reference: either brand (competitor_id NULL) or competitor (competitor_id NOT NULL)
  analysis_type TEXT NOT NULL CHECK (analysis_type IN ('brand', 'competitor')),
  competitor_id UUID REFERENCES competitors(id) ON DELETE CASCADE, -- NULL for brand analysis
  
  -- Cache entity name for query performance (denormalized for speed)
  entity_name TEXT NOT NULL, -- Brand name or competitor name
  entity_domain TEXT, -- Brand domain or competitor domain (cached)
  ...
  -- Constraints
  CHECK (
    -- If analysis_type is 'competitor', competitor_id must be NOT NULL
    -- If analysis_type is 'brand', competitor_id must be NULL
    (analysis_type = 'competitor' AND competitor_id IS NOT NULL) OR
    (analysis_type = 'brand' AND competitor_id IS NULL)
  )
);
...
CREATE INDEX idx_sentiment_analysis_competitor_id ON sentiment_analysis(competitor_id) WHERE competitor_id IS NOT NULL;
CREATE INDEX idx_sentiment_analysis_type ON sentiment_analysis(analysis_type);
...
COMMENT ON COLUMN sentiment_analysis.competitor_id IS 'Foreign key to competitors table. NULL for brand analysis, NOT NULL for competitor analysis';
COMMENT ON COLUMN sentiment_analysis.entity_name IS 'Cached entity name for performance. Denormalized from projects.brand_name or competitors.name';
COMMENT ON COLUMN sentiment_analysis.entity_domain IS 'Cached entity domain for performance. Denormalized from projects.client_url or competitors.domain';
